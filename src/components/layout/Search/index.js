...
function Search() {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8080/api/v1/product/seacrhByName?q=${encodeURIComponent(searchValue)}`)
            .then((res) => res.json())
            .then((product) => {
        ...
        <TippyHeadLess
            ...
            render={(attrs) => (
                <div className={cx('search_result')} tabIndex="-1" {...attrs}>
                    <h4 className={cx('search_title')}>Products</h4>
