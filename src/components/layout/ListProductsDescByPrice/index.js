...
import styles from './ListProductsDescByPrice.module.scss';
...
let listProducts = [];
function ListProductsDescByPrice() {
    const [products, setProducts] = useState([]);
    useEffect(() => {
        fetch('http://localhost:8080/api/v1/product/getAllOrderByPriceDesc')
            .then((res) => res.json())
            .then((product) => {
                setProducts(product);
            });
...
export default ListProductsDescByPrice;
