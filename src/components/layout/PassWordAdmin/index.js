...
import styles from './PassWordAdmin.module.scss';
...
function PassWordAdmin() {
    ...
    const handleAdd = async () => {
        if (newPass === conFirmPass) {
            fetch(
                `http://localhost:8080/api/v1/user/change-profile?token=${token}&username=${username}&oldPassword=${oldPass}&password=${newPass}`,
            );
            setShow(false);
